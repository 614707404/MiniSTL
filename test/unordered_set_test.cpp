...
#include "davis_unordered_set.h"
TEST(UnorderedSetTest, Constructor)
{
    davis::unordered_set<int> first;
    ...
    first.insert(50);
    first.insert(70);
    davis::unordered_set<int> second(first.begin(), first.end());
    EXPECT_EQ(second.size(), 4);
    davis::unordered_set<int> third(second);
    EXPECT_EQ(second.size(), 4);
}
...
TEST(UnorderedSetTest, Modifiers)
{
    davis::unordered_set<int> first;
    first.insert(0);
    first.insert(1);
    ...
    davis::unordered_set<int> second;
    first.swap(second);
    EXPECT_TRUE(first.empty());
    ...
    second.insert(3);
    EXPECT_EQ(second.size(), 4);
    EXPECT_EQ(second.count(0), 1);
    EXPECT_EQ(second.count(1), 1);
    EXPECT_EQ(second.count(2), 1);
