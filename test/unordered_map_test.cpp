...
#include "davis_unordered_map.h"
TEST(UnorderedMapTest, Constructor)
{
    davis::unordered_map<int,int> first;
    ...
    davis::unordered_map<int,int> second(first.begin(), first.end());
    EXPECT_EQ(second.size(), 4);
    davis::unordered_map<int,int> third(second);
    EXPECT_EQ(second.size(), 4);
}
TEST(UnorderedMapTest, Iterators)
{
    davis::unordered_map<int,int> first;
...
TEST(UnorderedMapTest, Modifiers)
{
    davis::unordered_map<int,int> first;
    first.insert(std::pair<int, int>(1, 10));
    first.insert(std::pair<int, int>(3, 30));
    ...
    davis::unordered_map<int,int> second;
    first.swap(second);
    EXPECT_TRUE(first.empty());
    ...
    second.insert(std::pair<int, int>(3, 30));
    second.insert(std::pair<int, int>(5, 50));
    second.insert(std::pair<int, int>(7, 70));
    EXPECT_EQ(second.size(), 4);
    EXPECT_EQ(second.count(1), 1);
    EXPECT_EQ(second[1], 10);
    EXPECT_EQ(second[3], 30);
    EXPECT_EQ(second[5], 50);
    EXPECT_EQ(second[7], 70);
    EXPECT_EQ(second.count(3), 1);
    EXPECT_EQ(second.count(5), 1);
    EXPECT_EQ(second.count(7), 1);
