#ifndef DAVIS_UNORDERED_MAP_H
#define DAVIS_UNORDERED_MAP_H
...
namespace davis
{
    ...
    public:
        unordered_map() : _M_ht(100, hasher(), key_equal()) {}
        ...
        template <class _InputIterator>
        unordered_map(_InputIterator __first, _InputIterator __last, size_type __n, const hasher &__hf, const key_equal &__eql)
            : _M_ht(__n, __hf, __eql)
        {
            _M_ht.insert_unique(__first, __last);
        }
        // copy
        unordered_map(const unordered_map &ust) : _M_ht(ust._M_ht) {}

        unordered_map &operator=(const unordered_map &ust)
        {
            _M_ht = ust._M_ht;
            return *this;
        }
        ...
        void swap(unordered_map &ust) { _M_ht.swap(ust._M_ht); }
        ...
        {
            return _M_ht.insert_unique(__val);
        }
        ...
        friend bool operator==(const unordered_map<Key,T, Hash, Pred, Alloc> &lhs,
    ...
    template <class Key, class T,class Hash, class Pred, class Alloc>
    void swap(const unordered_map<Key,T ,Hash, Pred, Alloc> &lhs,
              const unordered_map<Key, T,Hash, Pred, Alloc> &rhs)
    {
        lhs.swap(rhs);
    }
